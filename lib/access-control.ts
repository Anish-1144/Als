/** Shared access-control definitions for admin tab-wise permissions */
...
export const PAGE_ACCESS_MODULES: AccessModule[] = [
...
export const GLOBAL_ACCESS_MODULES: AccessModule[] = [
...
/** @deprecated legacy bundled global permission — migrated in normalizePermissions */
const LEGACY_GLOBAL_MODULE_IDS = ["navbar", "footer", "popup"] as const;
...
export function buildEmptyPermissions(): PagePermissions {
...
export function buildFullPermissions(): PagePermissions {
  const permissions: PagePermissions = {};
  for (const mod of ACCESS_MODULES) {
...
export function normalizePermissions(
  ...
  for (const mod of ACCESS_MODULES) {
    ...
    if (level && level in LEVEL_RANK) {
      base[mod.id] = level as AccessLevel;
    }
  }
  ...
  if (legacyGlobal && legacyGlobal in LEVEL_RANK) {
    for (const id of LEGACY_GLOBAL_MODULE_IDS) {
      if (!input[id] || input[id] === "none") {
        base[id] = legacyGlobal as AccessLevel;
      }
    }
  }
  return base;
...
export function getEffectivePermissions(
  role: string,
  pagePermissions?: PagePermissions | Record<string, string> | null,
): PagePermissions {
...
export function hasModuleAccess(
  role: string,
  pagePermissions: PagePermissions | Record<string, string> | null | undefined,
  moduleId: string,
...
export const MODULE_ACCESS_HINTS: Partial<
  Record<string, Partial<Record<AccessLevel, string>>>
> = {
  leads: {
...
/** Map an admin pathname to its access module id */
export function resolveModuleFromPath(pathname: string): string | null {
  ...
  if (path.startsWith("/admin/legal") || path.startsWith("/admin/pages/privacy") ||
      path.startsWith("/admin/pages/terms") || path.startsWith("/admin/pages/accessibility") ||
