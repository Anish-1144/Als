...
export const CALCULATOR_TOOL_SLUGS = [
  "borrowing-capacity",
  "extra-repayments",
  "property-fees",
] as const;
...
const SHARED_RESOURCES: SectionHeader & { cards: ContentCard[] } = {
  badge: "",
...
function mergeFactorCards(defaults: FactorCard[], raw?: FactorCard[]): FactorCard[] {
  const len = Math.max(defaults.length, raw?.length ?? 0);
...
function mergeStrategies(defaults: StrategyItem[], raw?: StrategyItem[]): StrategyItem[] {
  const len = Math.max(defaults.length, raw?.length ?? 0);
  return Array.from({ length: len }, (_, i) => ({
    title: raw?.[i]?.title ?? defaults[i]?.title ?? "",
...
function mergeInfoBlocks(defaults: InfoBlock[], raw?: InfoBlock[]): InfoBlock[] {
  const len = Math.max(defaults.length, raw?.length ?? 0);
  return Array.from({ length: len }, (_, i) => ({
    title: raw?.[i]?.title ?? defaults[i]?.title ?? "",
...
function mergeStringList(defaults: string[], raw?: string[]): string[] {
...
export const CALCULATOR_PAGE_DEFAULTS: Record<string, CalculatorPageContent> = {
  "borrowing-capacity": {
    pageHeader: {
      title: "Borrowing Capacity Calculator",
      subtitle:
        "Calculate your borrowing capacity based on your income, expenses, and financial situation. Get an instant estimate of how much you could borrow for your home loan.",
    },
    iframe: {
    ...
    infoSection: {
      ...
      intro:
        "Your borrowing capacity is the maximum amount a lender will allow you to borrow based on your financial situation. Lenders assess several factors when determining how much you can borrow:",
      factorCards: [
        {
...
export function mergeCalculatorPageContent(slug: string, raw?: Partial<CalculatorPageContent>): CalculatorPageContent {
  const d = CALCULATOR_PAGE_DEFAULTS[slug] ?? CALCULATOR_PAGE_DEFAULTS["borrowing-capacity"];
  return {
...
export function getCalculatorAdminPath(slug: CalculatorSlug): string {
  if (slug === "calculator") return "/admin/calculators";
