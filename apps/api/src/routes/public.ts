...
router.get("/homepage", async (_req, res) => {
  ...
  if (!doc) return fail(res, 404, "NOT_FOUND", "Homepage not found");
  return ok(res, doc);
});
...
router.get("/loans/:slug", async (req, res) => {
  ...
  if (!loan) return fail(res, 404, "NOT_FOUND", "Loan not found");
  return ok(res, loan);
});
...
router.get("/popup", async (_req, res) => {
  ...
  if (!doc) return fail(res, 404, "NOT_FOUND", "Popup not found");
  return ok(res, doc);
});
