...
export default async function CalculatorsHubClient({
  content,
  pageHero,
}: {
  content: CalculatorsHubContent;
  ...
  return (
    <div className="font-sans">
      <section
        ...
        style={{ backgroundImage: `url('${pageHero.heroBackgroundImage}')` }}
      >
        <div className="absolute inset-0 bg-black/50" />
        <div className="relative z-10 text-center text-white px-6">
          <h1 className="text-5xl md:text-6xl font-besley font-medium mb-6">
            {pageHero.heroTitle}
          </h1>
          <p className="text-xl text-gray-200 max-w-3xl mx-auto">{pageHero.heroSubtitle}</p>
        </div>
      </section>
                ...
                return (
                  <Link key={index} href={calc.link ?? "#"} className="block">
                    <div className="group relative p-8 rounded-2xl transition-all duration-300 cursor-pointer h-full text-center bg-[#1d293d] text-white border border-gray-600 hover:border-[#00a69c] hover:shadow-xl">
                      ...
                      <h3 className="text-xl font-besley font-semibold mb-4 text-white group-hover:text-[#00a69c] transition-colors">
                        {calc.title}
                      </h3>
            ...
            {isSectionVisible(teamSection) && (
              <div className="text-center mb-16">
                ...
                {teamSection.subtitle && (
                  <p className="text-lg text-gray-400 max-w-3xl mx-auto">{teamSection.subtitle}</p>
                )}
              </div>
            )}
                ...
                <div className="bg-[#2d3544] rounded-2xl shadow-lg overflow-hidden border border-gray-600">
                  <div className="grid md:grid-cols-2">
                    <div className="relative h-72 md:h-auto">
                      <div
                        className="w-full h-full bg-no-repeat min-h-[450px]"
                        style={{
                          backgroundImage: `url('${founderSpotlight.imageUrl}')`,
                    ...
                    <div className="p-8 flex flex-col justify-center">
                      ...
                      <h3 className="text-2xl md:text-3xl font-besley font-semibold text-white mb-2">
                      ...
                      <Link
                        href={founderSpotlight.ctaLink}
                        className="inline-flex items-center gap-2 bg-[#00a69c] text-white px-6 py-3 rounded-lg font-medium hover:bg-[#008b83] transition-colors w-fit"
                      >
              ...
              <div className="grid md:grid-cols-3 gap-8">
                {sortedTeam.map((member, index) => (
                  <div
                    ...
                    className="bg-[#2d3544] rounded-2xl shadow-lg overflow-hidden border border-gray-600"
                  >
                    ...
                    <div className="p-6 space-y-2">
                      {member.phone && (
                        <div className="flex items-center gap-3">
                          <FaPhone className="w-4 h-4 text-[#00a69c] shrink-0" />
                          <a
                      ...
                      {member.email && (
                        <div className="flex items-center gap-3">
                          <FaEnvelope className="w-4 h-4 text-[#00a69c] shrink-0" />
                          <a
