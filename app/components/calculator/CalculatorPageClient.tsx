...
type FactorStyle = { icon: IconType; iconClass: string; bulletClass: string };

const FACTOR_CARD_STYLES: Record<string, FactorStyle[]> = {
  "borrowing-capacity": [
    { icon: FaDollarSign, iconClass: "text-[#00a69c]", bulletClass: "text-[#00a69c]" },
    { icon: FaChartLine, iconClass: "text-green-500", bulletClass: "text-green-500" },
  ],
  "extra-repayments": [
    { icon: FaDollarSign, iconClass: "text-purple-400", bulletClass: "text-purple-400" },
    { icon: FaTimeline, iconClass: "text-indigo-400", bulletClass: "text-indigo-400" },
  ],
  "property-fees": [
    { icon: FaFileContract, iconClass: "text-orange-600", bulletClass: "text-orange-600" },
    { icon: FaDollarSign, iconClass: "text-amber-600", bulletClass: "text-amber-600" },
    { icon: FaHouse, iconClass: "text-yellow-600", bulletClass: "text-yellow-600" },
    { icon: FaChartLine, iconClass: "text-lime-600", bulletClass: "text-lime-600" },
  ],
};
...
export default function CalculatorPageClient({
  ...
  const { pageHeader, iframe, disclaimer, infoSection, resourcesSection, governmentBenefits, specialOffer } =
    content;
  const factorStyles = FACTOR_CARD_STYLES[slug] ?? [];
  const strategiesUseChecks = (infoSection.strategies ?? []).every((s) => !s.title);
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid lg:grid-cols-3 gap-8">
              ...
              {isSectionVisible(disclaimer) && (
                <div className="bg-[#2d3544] border border-[#00a69c] rounded-2xl p-6">
                  <div className="flex items-start gap-3">
                    <div className="w-6 h-6 bg-[#00a69c] rounded-full flex items-center justify-center shrink-0 mt-0.5">
                  ...
                  <div className="space-y-4 text-gray-200">
                    ...
                    <div className="grid md:grid-cols-2 gap-6 mt-6">
                      {infoSection.factorCards.map((card, index) => {
                        const style = factorStyles[index];
                        const Icon = style?.icon;
                        const bulletClass = style?.bulletClass ?? "text-[#00a69c]";
                        return (
                            ...
                            {infoSection.strategies!.map((item, index) =>
                              item.title ? (
                                <div key={index} className="flex items-start gap-3">
                                  <span className="text-[#00a69c] font-bold shrink-0">{index + 1}.</span>
                                  <div>
                    ...
                    {(infoSection.extraBlocks ?? []).map((block, index) => (
                      <div
                        key={index}
                        className="bg-[#1d293d] border border-[#00a69c]/40 p-6 rounded-xl mt-6"
                      >
                    ...
                    <div className="flex items-center gap-3 text-white">
                      ...
                      <div>
                        ...
                        {resourcesSection.subtitle && (
                          <p className="text-xs text-white/70">{resourcesSection.subtitle}</p>
                        )}
                      </div>
                    </div>
                  </div>
                      ...
                      return (
                        <Link key={index} href={card.link ?? "#"} className="block group">
                          <div className="flex items-center gap-4 p-4 rounded-xl bg-[#1d293d] hover:bg-[#3d4759] transition-all duration-300 border border-gray-600">
                            <div className="w-12 h-12 bg-[#173ab7] rounded-lg flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
                              <Icon className="w-6 h-6 text-white" />
                            </div>
                            <div className="flex-1 min-w-0">
                              <h4 className="font-semibold text-white group-hover:text-[#00a69c] transition-colors">
                                {card.title}
                              </h4>
                              <p className="text-sm text-gray-400">{card.description}</p>
                            </div>
                            <FaArrowRight className="w-4 h-4 text-[#00a69c] opacity-0 group-hover:opacity-100 transition-opacity shrink-0" />
                          </div>
                        </Link>
                      );
                    })}
            ...
            <div className="space-y-6">
                  ...
                  <div className="p-6">
                    ...
                    <div className="space-y-3 text-sm text-gray-200">
                      {governmentBenefits.items.map((item, index) => (
                        <div key={index} className="flex items-start gap-2">
                          <div className="w-1.5 h-1.5 bg-green-600 rounded-full mt-1.5 shrink-0" />
                          <p>{item}</p>
                        </div>
                      ))}
                    </div>
                    <Link
                      href={governmentBenefits.ctaLink}
              ...
              <Link
                href="/calculator"
                className="block w-full bg-[#2d3544] text-white px-6 py-4 rounded-2xl font-semibold hover:bg-[#3d4759] transition-colors border border-gray-600 text-center"
              >
